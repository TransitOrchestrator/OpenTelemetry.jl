# Autogenerated using ProtoBuf.jl v1.1.1 on 2025-04-15T13:50:48.207
# original file: /var/folders/z8/3_xmt_5d7vs6hdclc083pvlh0000gn/T/jl_mRWRro/opentelemetry-proto-1.5.0/opentelemetry/proto/logs/v1/logs.proto (proto3 syntax)
...
@enumx LogRecordFlags LOG_RECORD_FLAGS_DO_NOT_USE=0 LOG_RECORD_FLAGS_TRACE_FLAGS_MASK=255
...
struct LogRecord <: AbstractProtoBufMessage
...
PB.default_values(::Type{LogRecord}) = (;time_unix_nano = zero(UInt64), observed_time_unix_nano = zero(UInt64), severity_number = SeverityNumber.SEVERITY_NUMBER_UNSPECIFIED, severity_text = "", body = nothing, attributes = Vector{opentelemetry.proto.common.v1.KeyValue}(), dropped_attributes_count = zero(UInt32), flags = zero(UInt32), trace_id = UInt8[], span_id = UInt8[], event_name = "")
PB.field_numbers(::Type{LogRecord}) = (;time_unix_nano = 1, observed_time_unix_nano = 11, severity_number = 2, severity_text = 3, body = 5, attributes = 6, dropped_attributes_count = 7, flags = 8, trace_id = 9, span_id = 10, event_name = 12)
...
function PB.decode(d::PB.AbstractProtoDecoder, ::Type{<:LogRecord})
    ...
    while !PB.message_done(d)
        ...
        elseif field_number == 12
            event_name = PB.decode(d, String)
        else
            PB.skip(d, wire_type)
        end
    end
    return LogRecord(time_unix_nano, observed_time_unix_nano, severity_number, severity_text, body[], attributes[], dropped_attributes_count, flags, trace_id, span_id, event_name)
end
...
function PB.encode(e::PB.AbstractProtoEncoder, x::LogRecord)
    ...
    !isempty(x.event_name) && PB.encode(e, 12, x.event_name)
...
function PB._encoded_size(x::LogRecord)
    ...
    !isempty(x.event_name) && (encoded_size += PB._encoded_size(x.event_name, 12))
    return encoded_size
end

struct ScopeLogs <: AbstractProtoBufMessage
...
struct ResourceLogs <: AbstractProtoBufMessage
...
struct LogsData <: AbstractProtoBufMessage
